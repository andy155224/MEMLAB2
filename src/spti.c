...

{
	...
	if (argc == 8 || argc == 10) {
		...
		if (!strcmp(argv[1], "--disk")) {
			StringCbPrintf(string, sizeof(string), "\\\\.\\PhysicalDrive%s", argv[2]);	// Disk number
		}
		else {
			printf("把计岿粇莱赣璶琌--disk\n");
			return;
		}
		...
		if (!strcmp(argv[3], "--read")) {
			...
			printf("把计岿粇莱赣璶琌--read┪\n");
			return;
		}

		// lba
		if (!strcmp(argv[4], "--lba")) {
			start_lba = atoi(argv[5]);	// lba num
		}
		else {
			printf("把计岿粇莱赣璶琌--lba\n");
			return;
		}

		// sector count
		if (!strcmp(argv[6], "--sector_cnt")) {
			sectorCnt = atoi(argv[7]);	// sector count
		}
		else {
			printf("把计岿粇莱赣璶琌--sector_cnt\n");
			return;
		}
		...
		if (mode == 1) {
			...
			if (!strcmp(argv[8], "--data")) {
				pattern = (int)strtol(argv[9], NULL, 16);
			}
			else {
				printf("把计岿粇莱赣璶琌--data\n");
				return;
			}
		}
	}
		...
		printf("把计计秖岿粇\n");
		printf("Example:\n");
		printf("spti.exe PhysicalDrive2 r\n");
		return;
	}
	...
	if (mode == 1) {
		//int i = 0;
		FillMemory(dataBuffer, sectorSize, pattern);
		while (sectorCnt--) {
			//i = (int)start_lba;
			...
			sptdwb.sptd.Cdb[0] = 0x2A; //opcode 0x2A -> write
